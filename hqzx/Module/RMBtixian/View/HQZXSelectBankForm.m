...
@interface HQZXSelectBankController : UIViewController
@property (nonatomic, strong) HQZXSelectBankForm *actionView;
@end

@implementation HQZXSelectBankController
...
@interface HQZXSelectBankForm() {
    ...
    HQZXSelectBankController *actionSheetController;
    UIPickerView *mypickerView;
    NSMutableArray* aryBanks;
    HQZXCountry *countryNew;
}
@end
@implementation HQZXSelectBankForm
- (instancetype)initWithPlaceHolder:(NSString*) placeHolder {
    self = [super init];
    if (self) {
        placeHolderText = placeHolder;
        _bank = [[HQZXBank alloc] init];
    }
    return self;
}
...
- (IBAction)beSure:(id)sender {
    ...
    if (weakself.beSureComp) {
        weakself.beSureComp(_bank);
    }
    //    }];
}

- (void) showAction{
    aryBanks = [NSMutableArray array];
    ...
    if(dicPorts.count>0){
        ...
        for(NSDictionary *countryDic in countrys){
            HQZXBank *bank = [[HQZXBank alloc]init];
            bank.bank_id = StrValueFromDictionary(countryDic, @"id");
            bank.bank_name = StrValueFromDictionary(countryDic, @"name");
            bank.bank_ename = StrValueFromDictionary(countryDic, @"ename");
            [aryBanks addObject:bank];
        }
    }
    ...
    CGSize textSize = JHTCalcStringSizeWithFontSize(@"AAAAAAAAAAAAAAAAAAA", 13);
    ...
    actionSheetController = [[HQZXSelectBankController alloc] init];
