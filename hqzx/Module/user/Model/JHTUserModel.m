...
@implementation JHTUserModel
SYNTHESIZE_SINGLETON_FOR_CLASS(JHTUserModel)
...
-(JHTUser*) currentUser {
    if (_currentUser == nil) {
        ...
        if (userData) {
            ...
            _currentUser = [[JHTUser alloc] init];
...
- (void) logout {
    ...
    [CommonUtils removeUMengAlias:[JHTUserModel sharedInstance].currentUser.userId type:@"userid" startFromTime: 1];
    _currentUser = nil;
...
-(void) getContactInfo:(Contact_Block) callback beginRequestNet:(Void_Block) beginRequestNet {
    ...
    if (!userData) {
        ...
        [[NetHttpClient sharedHTTPClient] request:@"/user_info" parameters:nil completion:^(id obj) {
            ...
            JHTUserContact *userContact = [[JHTUserContact alloc] init];
