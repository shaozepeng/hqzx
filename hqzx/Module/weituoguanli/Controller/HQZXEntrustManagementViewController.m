//
//  HQZXEntrustManagementViewController.m
//  hqzx
//
...
@implementation HQZXEntrustManagementViewController{
...
- (void)viewDidLoad {
    ...
    self.title = LocatizedStirngForkey(@"WEITUOGUANLI");
    ...
    [zhuBtn setTitle:LocatizedStirngForkey(@"LISHIWEITUO") forState:UIControlStateNormal];
...
- (void) refData:(Void_Block) completion{
    [[NetHttpClient sharedHTTPClient] request: @"/coins_entrust_record.json" parameters:@{@"types":@"2",@"next_id":@"0", @"symbol": _sysId,@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
        if (obj==nil) {
            [self.view makeToast: LocatizedStirngForkey(@"LIANJIEFUWUQISHIBAI") duration: 0.5 position: CSToastPositionCenter];
            if (completion) {
                completion();
...
-(void)createScrollView{
    ...
    _scroll.contentSize = CGSizeMake(SCREEN_WIDTH*2.5, SCREEN_HEIGHT-TOP_HEIGHT);
...
-(void)createTableTwoView{
    UITableView *tableTwoView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0 , SCREEN_WIDTH*2.5, SCREEN_WIDTH/8) style:UITableViewStyleGrouped];
    ...
    [self.myTwoTableView registerClass:[HQZXEntrustTableCell class] forCellReuseIdentifier:@"CustomHeaderOne"];
    [_scroll addSubview:self.myTwoTableView];
}
-(void)createTableView{
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.myTwoTableView.maxY , SCREEN_WIDTH*2.5, SCREEN_HEIGHT-TOP_HEIGHT - SCREEN_WIDTH/8) style:UITableViewStyleGrouped];
    ...
    [self.myTableView registerClass:[HQZXDataEnTableCell class] forCellReuseIdentifier:@"CustomHeaderTwo"];
...
-(void)history{
    HQZXHistoryEntrustManagementViewController *historyView=[[HQZXHistoryEntrustManagementViewController alloc]init];
