//
//  HQZXVirtualRecordViewController.m
//  hqzx
//
...
#import "HQZXVirtualRecordViewController.h"

@implementation HQZXVirtualRecordViewController{
...
- (void) moreWithCompletion:(Bool_Block) completion {
    [[NetHttpClient sharedHTTPClient] request: @"/coins_paycheck_record.json" parameters: @{@"symbol": _sysid,@"next_id":nextId?nextId:@"0",@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
        if (obj==nil) {
...
- (void) refData:(Void_Block) completion{
    [[NetHttpClient sharedHTTPClient] request: @"/coins_paycheck_record.json" parameters:@{@"symbol": _sysid,@"next_id":@"0",@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
        if (obj==nil) {
            [self.view makeToast: LocatizedStirngForkey(@"LIANJIEFUWUQISHIBAI") duration: 0.5 position: CSToastPositionCenter];
            if (completion) {
                completion();
...
-(void)createTableTwoView{
    ...
    [self.myTwoTableView registerClass:[HQZXXNRecordTableCell class] forCellReuseIdentifier:@"CustomHeaderOne"];
...
-(void)createTableView{
    ...
    [self.myTableView registerClass:[HQZXXNDataRecordTableCell class] forCellReuseIdentifier:@"CustomHeaderTwo"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([tableView isEqual:self.myTableView]){
        ...
        HQZXXNDataRecordTableCell *cellen = (HQZXXNDataRecordTableCell*) cell;
        if(btbData){
            cellen.time = StrValueFromDictionary(btbData, @"time");
            cellen.wallet = StrValueFromDictionary(btbData, @"wallet");
