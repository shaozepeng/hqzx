...
#import "HQZXCashViewController.h"

@implementation HQZXCashViewController{
...
- (void) moreWithCompletion:(Bool_Block) completion {
    [[NetHttpClient sharedHTTPClient] request: @"/coins_extract_record.json" parameters: @{@"symbol": _sysid,@"next_id":nextId?nextId:@"0",@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
        if (obj==nil) {
...
- (void) refData:(Void_Block) completion{
    [[NetHttpClient sharedHTTPClient] request: @"/coins_extract_record.json" parameters:@{@"symbol": _sysid,@"next_id":@"0",@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
        if (obj==nil) {
            [self.view makeToast: LocatizedStirngForkey(@"LIANJIEFUWUQISHIBAI") duration: 0.5 position: CSToastPositionCenter];
            if (completion) {
                completion();
...
-(void)createScrollView{
    ...
    _scroll.contentSize = CGSizeMake(SCREEN_WIDTH*1.8, SCREEN_HEIGHT-TOP_HEIGHT);
...
-(void)createTableTwoView{
    UITableView *tableTwoView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0 , SCREEN_WIDTH*1.8, SCREEN_WIDTH/8) style:UITableViewStyleGrouped];
    ...
    [self.myTwoTableView registerClass:[HQZXCashWiTableCell class] forCellReuseIdentifier:@"CustomHeaderOne"];
    [_scroll addSubview:self.myTwoTableView];
}
-(void)createTableView{
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.myTwoTableView.maxY , SCREEN_WIDTH*1.8, SCREEN_HEIGHT-TOP_HEIGHT- SCREEN_WIDTH/8) style:UITableViewStyleGrouped];
    ...
    [self.myTableView registerClass:[HQZXDataCashWiTableCell class] forCellReuseIdentifier:@"CustomHeaderTwo"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([tableView isEqual:self.myTableView]){
        ...
        HQZXDataCashWiTableCell *cellen = (HQZXDataCashWiTableCell*) cell;
        if(btbData){
            cellen.time = StrValueFromDictionary(btbData, @"time");
            cellen.wallet = StrValueFromDictionary(btbData, @"extract_address");
            cellen.quantity = StrValueFromDictionary(btbData, @"quantity");
            cellen.fee = StrValueFromDictionary(btbData, @"procedures");
            cellen.status_name = StrValueFromDictionary(btbData, @"status_name");
        }
