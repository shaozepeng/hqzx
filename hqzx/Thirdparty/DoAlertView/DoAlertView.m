       ...
       customdo:(DoAlertBtnHandler)docustom
{
    ...
    [self customBottnTileShowAlertView];
}
...
- (void)customShowAlertView
{
    ...
    _vAlert.backgroundColor = UIColorFromRGB(0xC7C7C7);
    ...
    if (_strAlertTitle != nil && _strAlertTitle.length > 0)
    {
        ...
        [self setLabelAttributesTitle:lbTitle];
        ...
        dHeight = vTitle.frame.size.height;
    }

    // Body ---------------------------------------------------------------------------------------------------------
    [_vAlert addSubview:_bodyView];
    double dContentOffset = [self addContent:_bodyView];
    _bodyView.frame = CGRectMake(0, dHeight, _vAlert.frame.size.width,
                             dContentOffset + _bodyView.height  + 0.5);
    dHeight += _bodyView.frame.size.height;
    ...
    if (_strNoBtn!=nil)
    {
        ...
        btNo.tag = DO_NO_TAG;
        btNo.titleLabel.font = DO_TEXT_FONT;
        [btNo setTitle: _strNoBtn forState: UIControlStateNormal];
        [btNo setTitleColor:DO_NO_COLOR forState: UIControlStateNormal];
//        [btNo setImage:[UIImage imageNamed:@"close.png"] forState:UIControlStateNormal];
    ...
    if (_strYesBtn!=nil)
    {
        ...
        btYes.titleLabel.font = DO_TEXT_FONT;
        [btYes setTitle: _strYesBtn forState: UIControlStateNormal];
        [btYes setTitleColor:DO_YES_COLOR forState: UIControlStateNormal];
...
- (void)customBottnShowAlertView
{
    CustomRowView *costom = [CustomRowView userdInstance];
    UIView *noticeView=[costom customView:_btnArray];
    costom.Custom_Block=_docustom;
    _bodyView=noticeView;
...
- (void)customBottnTileShowAlertView
{
    CustomRowView *costom = [CustomRowView userdInstance];
    UIView *noticeView=[costom customView:_btnArray];
    costom.Custom_Block=_docustom;
...
- (void)showAlertView
{
    ...
    if (_doYes != nil)
    {
        ...
        if(_strYesBtn){
            ...
            [btYes setTitle: @"是的" forState: UIControlStateNormal];
        }
        [btYes setTitleColor:DO_YES_COLOR forState: UIControlStateNormal];
        //        [btYes setImage:[UIImage imageNamed:@"check.png"] forState:UIControlStateNormal];
        [btYes addTarget:self action:@selector(buttonTarget:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setLabelAttributesTitle:(UILabel *)lb
{
    lb.backgroundColor = [UIColor clearColor];
    lb.textAlignment = NSTextAlignmentCenter;
    lb.numberOfLines = 0;
