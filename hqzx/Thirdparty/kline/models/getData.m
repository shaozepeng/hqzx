...
//        }else{
//            NSString *dayK = [NSString stringWithFormat:@"%d",60*24];
//            [[NetHttpClient sharedHTTPClient] request: @"/kline.json" parameters: @{@"symbol":sysid,@"limit":@"100",@"step":dayK,@"auth_key":[HQZXUserModel sharedInstance].currentUser.auth_key} completion:^(id obj) {
//                if (obj==nil) {
//                    return;
//                }
...
//                NSLog(@"Error: %@", error);
//            }];
//            NSString *path = [[NSBundle mainBundle] pathForResource:@"table" ofType:@"csv"];
...
-(void)changeData:(NSArray*)lines type:(NSString*)refType{
    ...
    for (idx = newArray.count-1; idx > 0; idx--) {
        NSArray   *arr = [newArray objectAtIndex:idx];
...
//        CGFloat idxLocation = [lines indexOfObject:arr];
        // MA5
        [item addObject:[NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idx, MA5)]]]; // 前五日收盘价平均值
        // MA10
        [item addObject:[NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idx, MA10)]]]; // 前十日收盘价平均值
        // MA20
        [item addObject:[NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idx, MA20)]]]; // 前二十日收盘价平均值
        // 前面二十个数据不要了，因为只是用来画均线的
        NSString *timeg = [arr objectAtIndex:0];
        NSDate *nd = [NSDate dateWithTimeIntervalSince1970:[timeg integerValue]];
        ...
        [category addObject:dateString]; // date
        [data addObject:item];
    }
	if(data.count==0){
