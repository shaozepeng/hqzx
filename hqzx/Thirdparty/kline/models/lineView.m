...
-(void)initSet{
    ...
    self.xWidth = SCREEN_WIDTH-SCREEN_WIDTH/5; // k线图宽度
...
-(void)drawBox{
    // 画个k线图的框框
    if (mainboxView==nil) {
        mainboxView = [[UIView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/8, 20, self.xWidth, self.yHeight)];
...
//        [mainboxView addGestureRecognizer:pinchGesture];
    ...
    if (volMaxValueLab==nil) {
        volMaxValueLab = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/8-30, mainboxView.frame.size.height+mainboxView.frame.origin.x
                                                                   , 30, self.font.lineHeight)];
